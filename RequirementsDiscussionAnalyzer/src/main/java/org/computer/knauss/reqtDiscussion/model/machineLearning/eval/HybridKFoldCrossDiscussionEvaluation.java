...
public class HybridKFoldCrossDiscussionEvaluation {
	...
	public HybridKFoldCrossDiscussionEvaluation() {
		...
		try {
			...
			clarifClassifier.init(new File("clarifClassifier.txt"));
			clarifClassifier.setAutosave(false);
			coordClassifier = new NewBayesianClassifier();
		...
		for (Discussion[] ds : bucket) {
			for (Discussion d : ds) {
				for (DiscussionEvent de : d.getDiscussionEvents()) {
					...
					if (de.isClassified()) {
						if (de.isInClass()) {
							this.clarifClassifier.learnInClass(de.getContent());
							this.coordClassifier.learnNotInClass(de
									.getContent());
							// System.out.print('!');
						} else {
		...
		for (Discussion[] ds : bucket) {
			for (Discussion d : ds) {
				for (DiscussionEvent de : d.getDiscussionEvents()) {
					...
					double clarifConfidence = clarifClassifier.classify(de
							.getContent());
					double coordConfidence = coordClassifier.classify(de
							.getContent());
					dec.setClassifiedby(clarifClassifier.getClass()
							.getSimpleName());
					// dec.setConfidence(confidence);
					dec.setWorkitemcommentid(de.getID());

					if (clarifClassifier.getMatchValue() < clarifConfidence
							&& coordClassifier.getMatchValue() > coordConfidence) {
						dec.setClassification("clarif");
						// System.out.print('!');
					} else if (clarifClassifier.getMatchValue() < clarifConfidence
							&& coordClassifier.getMatchValue() < coordConfidence) {
						dec.setClassification("both");
						// System.out.print('.');
					} else {
		...
		for (Bucket bucket : buckets) {
			for (Discussion[] complexTrajectory : bucket) {
				if (considerDiscussion(complexTrajectory)) {
					...
					IClassificationFilter.NAME_FILTER
							.setName(this.clarifClassifier.getClass()
									.getSimpleName());
